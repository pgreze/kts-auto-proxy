...
export function rewriteRepositories(lines, mavenCentralProxy, reposToProxies) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (isUrlMavenCentral(repositoryLine)) {
            ...
            lines[i] = `@file:Repository("${mavenCentralProxy}")${repositoryLine.rest}`
            continue
        }
    ...
    if (!mavenCentralRepositoryFound) {
        const firstDependsOnIndex = lines.findIndex(line => line.includes('@file:DependsOn'))
        if (firstDependsOnIndex === -1) {
            core.debug('No @file:DependsOn found, maven central repository will not be added')
        } else {
            core.debug(`Add maven central repository before @file:DependsOn`)
            lines.splice(firstDependsOnIndex, 0, `@file:Repository("${mavenCentralProxy}")`)
        }
    }
